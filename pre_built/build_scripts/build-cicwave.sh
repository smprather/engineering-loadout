...
# other already-bundled shared wheels are reused from pre_built/<platform>/wheels.
#
# Usage:
#   pre_built/build_scripts/build-cicwave.sh --tag 0.5.2
#   pre_built/build_scripts/build-cicwave.sh --tag 0.5.2 --source /path/to/checkout

set -euo pipefail

REPO="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
PLATFORM="el8.x86_64.glibc2p28"
WHEELS_DIR="$REPO/pre_built/$PLATFORM/wheels"
PATCH="$REPO/pre_built/build_scripts/cicwave/0001-port-pyside6-to-pyqt6.patch"
...
echo
echo "Done. Next:"
echo "  python3 -m json.tool pre_built/packages.json >/dev/null   # if you edited the registry"
...
echo "  git add pre_built/$PLATFORM/wheels pre_built/build_scripts/cicwave pre_built/build_scripts/build-cicwave.sh"
