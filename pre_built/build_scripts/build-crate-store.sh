...
#   1. Reads pre_built/build_scripts/rust-crate-list.txt (top-level seed crates).
...
LIST="$REPO/pre_built/build_scripts/rust-crate-list.txt"
OUT_DIR="$REPO/rust"
...
# Match strip_all_elf_binaries' CHUNK_THRESHOLD so no committed file trips
# GitHub's 50 MB warning. rust/ is top-level (not under pre_built/), so the
...
# 3-digit), matching strip_all_elf_binaries' split.
...
echo "   (archive rust/crate-store.tar.bz2, installed by install_crate_store)."
