...
_loadout_complete() {
    ...
    local tags="agent automation backup benchmark browser build cache clipboard compat compiler compress core data dedup def dev diff disk docs document eda editor elf env file font format git gobject gui hotkeys hpc javascript lang lef liberty library lint llvm logic lsp lua markdown math minimizer monitor multiplexer nav net network notebook nvim opengl optimizer parser pkg plot profiler prompt python reference runtime rust scientific screenshot scripting search security shell simulator spice ssh sync system terminal text tmux toolchain tui vcd viewer visualization waveform"
