...
# uv_tool packages whose LAUNCHER must export PYTHONPATH, keyed by uv_tool
# distribution name. Value = the registry `bins` entry to wrap and the
# directory (under the tool venv's site-packages, derived at install time) that
# must become importable.
#
# WHY: certain flows run helper PYTHON scripts through a *different* interpreter
# than the tool's own venv -- LibreLane's `odbpy`/`pyosys` steps shell out to
# `openroad -python` and `yosys -y`, both of which execute the script with the
# EMBEDDED portable-python (librelane/steps/odb.py sets PYTHONPATH itself, but
# only to append its scripts dir). portable-python is deliberately minimal, so
# `import click` / `import rich` / `import yaml` inside those scripts fail with
# "No module named 'click'" and the flow dies at the first Odb.* step. The
# upstream image has no such gap because its openroad embeds a python whose
# site-packages carries the flow's dependencies.
#
# A venv's site-packages is NOT on a subprocess's sys.path -- only PYTHONPATH
# crosses that boundary -- so the launcher prepends it. Prepending (not
# replacing) keeps any caller PYTHONPATH intact, which LibreLane relies on when
# it appends its own scripts dir.
_UV_TOOL_LAUNCHER_PYTHONPATH = {
    "librelane": {
        "launcher": "librelane",
        # Relative to the tool venv root (<tools>/<dist>): every wheel's top
        # level lands here.
...
def _wrap_uv_tool_launchers(pkg_name, bin_home, tools_home):
    ...
    uv creates <bin_home>/<name> as a symlink to the venv's own console script
    (whose shebang is the venv python). We replace that symlink with a tiny sh
    wrapper that exports PYTHONPATH=<venv site-packages> and then execs the
    original script, which stays exactly where uv put it.

    Returns the list of launcher names actually wrapped.
    """
    spec = _UV_TOOL_LAUNCHER_PYTHONPATH.get(pkg_name)
    if not spec:
        return []
    ...
    site_packages = os.path.join(venv_root, spec["site_packages"])
    if not os.path.isdir(site_packages):
        warn(
            f"not wrapping {launcher} launcher: {site_packages} does not exist "
            "(helper scripts may fail to import their dependencies)"
        )
        return []
    # The real console script: follow the symlink uv made. Refuse to wrap a
    # second time (a re-run over an already-wrapped launcher), which would nest
    # wrappers and, worse, could end up exec'ing itself forever.
    ...
    try:
        with open(target, "rb") as fh:
            if b"_LOADOUT_PYTHONPATH_WRAPPER" in fh.read(4096):
                return []
    except OSError as exc:
        warn(f"could not inspect uv launcher target {target}: {exc}")
        return []
    wrapper = (
        "#!/bin/sh\n"
        "# _LOADOUT_PYTHONPATH_WRAPPER -- generated by the loadout installer.\n"
        "#\n"
        "# Exports the tool venv's site-packages on PYTHONPATH before exec'ing\n"
        "# the real launcher. Helper scripts this tool runs through ANOTHER\n"
        "# interpreter (e.g. LibreLane's `openroad -python` odbpy steps, which\n"
        "# execute under the embedded portable-python) resolve their imports\n"
        "# (click/rich/yaml) from there. Prepended, never replacing, so the\n"
        "# tool's own PYTHONPATH handling keeps working. Regenerated on every\n"
        "# install; do not edit by hand.\n"
        ...
        for tool_name, pkg_name, install_spec in tools_to_install:
            ...
            try:
                ...
                if proc.returncode != 0:
                    ...
                    _tools_home = os.path.join(_env["XDG_DATA_HOME"], "uv", "tools")
                    for launcher in _wrap_uv_tool_launchers(pkg_name, _env["XDG_BIN_HOME"], _tools_home):
                        _vprint(
...
# --- system-newer report (doctor, read-only) -----------------------------------
#
# Phase 4 of the system-package-precedence methodology: report-only. Installing
# a payload binary into ~/.local/bin shadows any same-named system binary that
# sorts later on PATH -- including when the system copy is NEWER. This section
# lists payload bins for which a newer system copy exists. It never changes
# install behavior and never fails doctor: unparseable versions on either side
# are silently skipped, i.e. fail closed toward installing.
#
# Probe flags are intentionally (--version, -V) only: -v means "verbose" or
# "vi mode" for several tools and can hang or spew. Detection is PATH-based
# and fully offline. Loadout-managed bin dirs are excluded by realpath, so the
# installer never compares the payload against itself.

_SYSTEM_VERSION_FLAGS = ("--version", "-V")
_SYSTEM_PROBE_TIMEOUT = 4
_VERSION_TOKEN = re.compile(r"(\d+(?:\.\d+)+|\d+)")


def _ver_tuple(text):
    """First version-like token in text as an int tuple, else None.

    A match preceded by a letter is skipped (so the "64" in "base64" never
    parses as a version), except after "v"/"V" ("v1.2.3" is a version).
    """
    if not text:
        return None
    for m in _VERSION_TOKEN.finditer(str(text)):
        prev = text[m.start() - 1] if m.start() > 0 else ""
        if prev.isalpha() and prev not in ("v", "V"):
            continue
...
def _ver_cmp(a, b):
    """Compare int version tuples with zero-padding; returns -1, 0 or 1."""
    n = max(len(a), len(b))
    pa = a + (0,) * (n - len(a))
    pb = b + (0,) * (n - len(b))
    return (pa > pb) - (pa < pb)
...
def _find_system_bin(name, path_str, skip_dirs):
    """First PATH copy of name outside skip_dirs, else None.

    skip_dirs holds dir realpaths; the candidate file's own realpath is
    checked too, so a symlink in /usr/bin pointing back into ~/.local/bin
    does not count as a system copy.
    """
    for d in path_str.split(os.pathsep):
        if not d:
            continue
        try:
            if os.path.realpath(d) in skip_dirs:
                continue
            cand = os.path.join(d, name)
            if not (os.path.isfile(cand) and os.access(cand, os.X_OK)):
                continue
            if os.path.dirname(os.path.realpath(cand)) in skip_dirs:
                continue
        except OSError:
            continue
        return cand
...
def _probe_bin_version(path):
    """Version tuple from `<bin> --version|-V` (stdout or stderr), else None."""
    for flag in _SYSTEM_VERSION_FLAGS:
        try:
            proc = subprocess.run(
                [path, flag],
                capture_output=True,
        ...
        if not text:
            continue
        ver = _ver_tuple(text.splitlines()[0])
        if ver is not None:
            return ver
    return None


def _system_newer_rows(registry, path_str=None, loadout_dirs=None):
    """Rows (pkg, bin, payload_ver, sys_path, sys_ver, standing) where a newer
    system copy of a payload bin exists. Read-only; unparseable versions on
    either side are skipped. `standing` is payload-wins (loadout bin dir sorts
    first on PATH, so installing shadows the system copy), system-wins, or
    off-path (loadout bin dir not on PATH at all).
    """
    if path_str is None:
        path_str = os.environ.get("PATH", "")
    if loadout_dirs is None:
        loadout_dirs = _default_loadout_bin_dirs()
    path_dirs = []
    for d in path_str.split(os.pathsep):
        if not d:
            continue
        try:
            path_dirs.append(os.path.realpath(d))
        except OSError:
            continue
    rows = []
    for pkg in sorted(registry):
        entry = registry[pkg]
        payload_ver = _ver_tuple(entry.get("version", ""))
        if payload_ver is None or not entry.get("bins"):
            continue
        for b in entry["bins"]:
            sys_path = _find_system_bin(b, path_str, loadout_dirs)
            if sys_path is None:
                continue
            sys_ver = _probe_bin_version(sys_path)
...
def cmd_doctor(args, registry, repo_dir):
    ...
    # System-newer report (read-only): payload bins for which a NEWER system
    # copy exists on PATH. Installing would shadow payload-wins rows via
    # ~/.local/bin. Never fails doctor; see _system_newer_rows.
    newer_rows = _system_newer_rows(registry)
    if newer_rows:
        print(
            f"System binaries newer than payload ({len(newer_rows)}) -- installing shadows payload-wins rows via ~/.local/bin:"
        )
        for pkg, b, pver, spath, sver, standing in newer_rows:
            print(f"  {pkg}: {b} payload {pver} < {spath} {sver} [{standing}]")
    else:
