...
This repository is engineering-loadout: an offline-first, no-root package manager for EE Linux/Windows environments. The installer is `./loadout` (Python 3.6+), driven by `pre_built/packages.json` (`schema_version: 3`). Prefer changes that preserve RedHat/Alma/RHEL 7/8/9, Suse, WSL, Windows PowerShell, and locked-down corporate machines.
...
- Linux installer is the Python 3.6-compatible executable `loadout`. It checks the Python version before running, resolves the repo from the script path, and must work when invoked from outside the repo root; `tests/install_linux_tmp_home` runs it from `/tmp` to catch regressions.
