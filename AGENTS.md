...
- **`pre_built/build_scripts/test-prebuilt-binaries` is the release gate.** Run `./release --dry-run` before tagging. The script does a full `./engineering-loadout --dest-dir /tmp/...`, probes each binary for loader failures and signal crashes, checks editor runtime sentinels, runs installed `nvim` headless against the installed runtime, and asserts portable Python does not shadow `python3`/`pip3`.
...
- Fresh Neovim config must start on locked-down machines. If `lazy.nvim` is missing and `git` cannot clone it, `nvim/init.lua` disables the plugin layer cleanly (`vim.g.dotfiles_plugins_enabled = false`) instead of erroring.
