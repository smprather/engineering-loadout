...
- Platform runtime archives live under `pre_built/<platform>/runtime/`. `helix.tar.bz2` installs to `~/.local/share/helix/runtime` with `runtime/tutor` as sentinel. `vim92.tar.bz2` installs to `~/.local/share/vim/vim92` with `filetype.vim` as sentinel. `nvim.tar.bz2` installs to `~/.local/share/nvim/runtime` with `filetype.lua` as sentinel. `st.tar.bz2` installs to `~/.local/share/terminfo/s/` with `st-256color` as sentinel. `mesa3d_libs.tar.bz2` is chunked as `.part-000..002` and installs Mesa under `~/.local/lib64` / `~/.local/lib64/dri`. `wezterm.tar.bz2` installs a wrapper at `~/.local/bin/wezterm` and real ELF at `~/.local/lib/wezterm/wezterm.bin`.
...
- Bundled terminfo belongs under `<prefix>/share/terminfo`, not `~/.terminfo`. `bash/global/bashrc` prepends `$HOME/.local/share/terminfo` to `TERMINFO_DIRS` so ncurses finds staged/shared installs of `st-256color` and any future shipped terminfo entries.
