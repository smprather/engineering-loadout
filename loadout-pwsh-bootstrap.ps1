...
.DESCRIPTION
    ...
    vendored PowerShell ZIP from payload/windows.x86_64/powershell into
...
function Get-LoadoutBundledPowerShellArchive {
    ...
    $roots = @(
        (Join-Path $RepoDir 'payload\windows.x86_64\powershell'),
        (Join-Path $RepoDir 'payload\windows\powershell')
    )
...
function Install-LoadoutBundledPowerShell {
    ...
    if (-not $archive) {
        throw "Bundled PowerShell archive not found. Expected payload\windows.x86_64\powershell\PowerShell-*-win-x64.zip or .zip.part-000."
    }
