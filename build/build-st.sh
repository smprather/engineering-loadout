...
#
# Patch order (both are applied to a fresh tree, in this order):
#   1. upstream undercurl + the three st.c/st.info fixups above
#   2. build/st/0001-runtime-xresources-config.patch -- the loadout runtime-config
#      patch: st merges an X resource database from the colon-separated file list
#      in $ST_XRESOURCES (later file wins) over RESOURCE_MANAGER, and re-reads it
#      on SIGUSR1 / Ctrl+Shift+R. Without it, changing st's font or colors means
#      recompiling, which the machines we deploy to cannot do.
#
# Packaging is a wrapper split (gvim/ngspice style):
#   bin/st        POSIX-sh wrapper (build/st/st) -- names the ST_XRESOURCES chain
...
PYEDIT
    ...
    # Loadout runtime-config patch: st reads $ST_XRESOURCES (colon-separated
    # layer chain, later file wins) over RESOURCE_MANAGER, and re-reads it on
    # SIGUSR1 / Ctrl+Shift+R. Without this, changing the font or colors on a
    # deployed machine means recompiling -- which farm nodes cannot do.
...
# st's Makefile only copies config.def.h -> config.h when config.h is absent,
# and `make clean` does not remove it. A stale config.h from an earlier build
# silently drops every config.def.h change (the reload shortcut, the writable
# colorname[]), producing a binary that looks fine and ignores its config.
rm -f config.h
...
# bin/st is a POSIX-sh wrapper that names the ST_XRESOURCES layer chain and
# execs the real ELF at bin/st.bin; st-reload signals SIGUSR1 to running
# instances. Plain scripts -- no strip, no patchelf, just bzip2.
...
# binary file per st-* variant) and pack them under ./share/terminfo/s/, which
# is where the registry's sentinel (share/terminfo/s/st-256color) and
# install_to (~/.local) expect them. envs/bash/global/bashrc prepends
# ~/.local/share/terminfo to TERMINFO_DIRS so ncurses finds st-256color there.
# (This staged ./.terminfo/ once, which disagreed with both the shipped archive
# and the sentinel -- the next tag bump would have installed to ~/.local/.terminfo
# and failed its own sentinel check.)
...
echo ""
echo "Next steps:"
echo "  $REPO/build/verify-binaries st"
