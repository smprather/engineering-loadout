...
# JSON-parse rather than regex: a multi-line entry silently defeated the old
# re.sub (no DOTALL), leaving the version stamp a no-op that still printed
# success.
python3 -c "
import json, sys
...
with open(path) as f:
    data = json.load(f)
data['packages']['nvim']['version'] = ver
...
echo "  git add payload/el8.x86_64.glibc2p28/bin/nvim.bz2 \\"
echo "          payload/el8.x86_64.glibc2p28/runtime/nvim.tar.bz2 \\"
echo "          .strip-manifest payload/packages.json"
echo "  git commit -m 'feat(payload): nvim ${ver} stable EL8 source build'"
