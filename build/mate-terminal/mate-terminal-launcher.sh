#!/bin/sh
# mate-terminal launcher -- loadout shanghai bundle.
# Composed of: XDG/GSETTINGS header + build/gui-wrapper-env.sh + build/gtk3-launcher-env.sh.
# To regenerate: re-run this composition (header below) after fragment changes.
#
# Resolves the loadout install prefix from the installed path so
# the bundled gsettings schemas are discoverable, and forces the keyfile
# GSettings backend so no running dconf-service or session bus is needed.
...
# keyfile persists at ~/.config/glib-2.0/settings/keyfile; no D-Bus needed.
export GSETTINGS_BACKEND=keyfile
exec "$prefix/bin/mate-terminal.bin" "$@"
