...
**WezTerm runtime behavior (shanghai bundle)**: WezTerm `20260618_095146_c10636f3` is bundled from `/usr/bin/wezterm` by `pre_built/build_scripts/build-wezterm.sh`. Bundle extracts to `~/.local/` and installs `bin/wezterm` (POSIX-sh wrapper), `lib/wezterm/wezterm.bin` (real ELF, RPATH includes `$ORIGIN/../../lib64`), desktop/metainfo files when present, and zsh completion when present. The wrapper derives prefix from installed `bin/..`, prepends Mesa lib paths, points GLVND at the bundled Mesa JSON, keeps the local portal restart workaround from `~/.local/bin/wezterm`, and execs `lib/wezterm/wezterm.bin`. It is the sample app for `mesa3d_libs`; install with `./loadout install wezterm` or `@gui-suite`.
