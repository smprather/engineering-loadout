...
tests/                      - Test suite (see tests/run-all)
  ...
  install-linux-tmp-home    - Full install into a temp HOME (fresh-user simulation)
...
Group expansion (`expand_groups`) recursive, cycle-detected. Synthetic groups expand at runtime (not stored in `packages.json`), in two symmetric pairs -- the `-all` suffix always means "with `optional` packages folded back in":
...
| `@envs` | every non-group, **non-`optional`** `kind == env` config bundle (the complement of `@shared`) |
| `@envs-all` | the same **plus** the optional env bundles (`env-tcsh`, `env-cargo`) |

**There is no bare `all`.** It used to mean "every *non-optional* package" -- the opposite of what the `-all` suffix means everywhere else -- so it was removed and now raises a `ResolverError` pointing at the replacements. Use `@engineering-loadout` for the full bundled set (an identical resolved set: its `members` start with `@shared`, `@envs`), or `@shared-all @envs-all` for truly everything. **`optional: true` is the only gating field** (there is no `default` field): flagged packages are skipped by the `@shared` / `@envs` / `@engineering-loadout` sweeps and install only when named explicitly, via an `-all` group, or when pulled by a group that lists them in `members` -- e.g. `surfer` (name-only), the `@rust` trio (`rust`, `rust-crate-store`, `env-cargo`, reachable via `@rust`), and `env-tcsh` (name-only or via `@envs-all`). Tools and env config bundles are **fully decoupled** -- no cross-`recommends` in either direction -- so `./loadout install @shared --dest-dir ...` and `./loadout install @envs` are clean complements needing no `--skip`/`--no-deps`. All four synthetic groups surface in `list --groups` and `describe <@group>` (driven by `_SYNTHETIC_GROUPS`) despite not being registry entries.
...
- Positional `PKG...` -- install exactly these packages/`@groups` (deps still walked unless `--no-deps`). There is no bare `all`; use `@engineering-loadout` (full bundled set) or `@shared-all @envs-all` (truly everything).
...
- `tests/run-all` -- single suite entry point. Default runs Tier 1 (py_compile, ruff, unit-resolver, registry-integrity, tar-safety, check-installer) plus Tier 2 (install-fonts-rejoin, install-modules, optional-packages, install-linux-tmp-home, install-split-shared-envs, completion-sync diff). `--fast` stops after Tier 1; `--container` adds Tier 3.
...
- `tests/install-split-shared-envs` is the production deployment shape: `@shared` into a temp non-home tree, then `@envs` into a separate temp HOME with `LOADOUT_CFG_SHARED_PREFIX=<shared>/local`; smokes bash/zsh startup, shared PATH, terminfo, WezTerm completions, and core tool startup.
