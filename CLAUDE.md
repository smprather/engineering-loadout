...
**parity-plot Python tool behavior**: `parity-plot` is non-optional (therefore included in `@shared`) and also lives in `@python-tools-extra`. It is pinned to stable upstream tag `v0.6.0` (`e98f08fecb7f250af508e0d81c9c94349cc6b1ea`); rebuild with `build/build-parity-plot.sh --tag v0.6.0`. The script builds the pure project wheel, exports runtime dependencies from upstream's lock, and downloads the whole EL8/cp314 wheel closure. It first validates/reuses the vendored lock closure, so a same-lock rebuild is offline; if a release tag has stale lock metadata, the builder runs `uv lock` only in the disposable checkout before exporting hash-locked requirements. NiceGUI is a core upstream dependency now, not a `uv_extras` entry, so `uv tool install parity-plot` installs both CLI and designer offline. Its loadout-only patch at `build/parity-plot/0001-inline-plotly-js-for-offline-html.patch` must apply cleanly, and is now a **single hunk**: it replaces upstream's CDN Plotly reference with `include_plotlyjs=True`, making generated HTML self-contained. It used to carry a second hunk rewriting `__version__`, which was pure fragility -- the build script already stamps the real tag into `parity_plot/__init__.py` with a `count != 1` assertion, so the hunk changed nothing and only added a context that broke on every upstream import re-sort (it did exactly that on the v0.6.0 bump). Keep the patch to the one line that carries meaning. This increases each HTML report by about 4.9 MiB but adds no new wheel payload because Plotly already ships its minified JS. PNG/SVG/PDF export uses Kaleido and still requires a compatible Chrome/Chromium already installed on the host; do not invoke a downloader during an offline install. **v0.6.0 is a TOML-only CLI**: `parity-plot plot` takes a config file (default `parity.toml`), no longer a CSV path. `parity-plot init` writes a fully-commented starter config and `parity-plot example` generates sample data and renders it. `tests/install-parity-plot` installs into a temp tree, verifies CLI and module versions, drives `plot` through a generated TOML config, exercises `init` and `example`, inspects the HTML for an embedded Plotly runtime and no CDN reference, imports the designer dependencies, and probes the local designer server where host policy permits loopback sockets. Its expected version is **read from `packages.json`** rather than hard-coded -- the literal went stale on every bump, and what the test needs to prove is that the installed artifact matches what the registry claims to ship.
