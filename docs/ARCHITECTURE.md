...
The installer is driven by `payload/packages.json` (`schema_version: 3`) -- a typed
...
./loadout list --groups editor cli              # group names matching either filter
...
See `build/ADDING_BINARIES.md` for the full workflow and per-tool build notes.
...
3. Cold bootstrap: decompresses
   `payload/<platform>/portable-python-*.tar.bz2` into
